...
#feb 21, 17
##im hitting on syntax of true app composition. 
very much like react. build app type and instantiate and run. these types are funcs composing thru index files. so, if serialized they can be kept in Mongo. the whole app can be stuffed their the same way, of course. but, this allows to put partial info defining type of app. A boilerplate in configuration, like drupal. the gui app can grab a an app out of collection and build folder struct of an app. Do you think there is any use for that? It can be done thru a zip file as well.

##i understood what i'm doing.
normally, for the lack of good time dimension syntax we separate the sequence of execution into space like scopes - a bootstrap func comes in top scope above anybody using it and so on. That creates a lot of files and dirs and boilerplate. spiceTime creates the syntax of time dimension eliminating that. The space gets srunk and time gets expanded but time is efficient. Its just a bunch of tick props in a type and value containers. It minimizes all the names to keep track of (my biggest problem, just gets too many). all time slice results are in a nice namespace which is time history of spiceNode. All local. Thats what makes those one liners possible. Instead of a bunch of dirs and index files you have a destruct statement and a one liner in a single module. Thats just the beginning out of the box if you dont do any custom types. Those will take you to the next level. Its beautiful.

thats what my confusion was. i was trying to be efficient and condense those scopes by being clever. it airnt worth it to be clever. gets complicated. Thios is why i started doing this w/o even realizing exactly why. just intuition.

## gravity is there but its as complicated as in spaceTime
particles (props) flow thru structure (spice) along the twisted lines of spice twisted by funcs and their reusable code units. Those are folds in space - the curvature. That curvature directs data flow between funcs. The data in timeSpice flows both up and down the tree-composition and prop passing. How is it useful i dont know but it is an abstraction to be aware of.
